...
app.delete("/teams/:teamId", authMiddleware, async (req, res) => {
  try {
    ...
    if (team.createdBy.toString() !== req.userId) {
      return res.status(403).json({ error: "You are not allowed to delete this team" });
    }
...
app.get("/teams/:teamId/requests", authMiddleware, async (req, res) => {
  try {
    ...
    if (team.createdBy.toString() !== req.userId) {
      return res.status(403).json({ error: "You are not allowed to view these requests" });
    }
...
// GET My Join Requests - FIXED: Moved outside and before :requestId routes
...
app.patch("/requests/:requestId/accept", authMiddleware, async (req, res) => {
  try {
    ...
    if (team.createdBy.toString() !== req.userId) {
      return res.status(403).json({ error: "You are not allowed to accept this request" });
    }
    ...
    await team.save();
    res.json({ message: "Join request accepted!", request });
  } catch (err) {
    console.log(err);
